...
const Courses = () => {
  ...
  const [currentEditingQuizId, setCurrentEditingQuizId] = useState<string | null>(null)
  ...
  const handleAddQuestion = () => {
    ...
    if (editingIndex !== null && editingIndex > -1) {
      ...
      const prevScore = Number(updated[editingIndex]?.score) || 0;
  ...
  const handleLoadQuizToEditor = (q: any) => {
    try {
      ...
      // Set time fields from existing quiz
      if (q?.startsAt) {
        const startDate = new Date(q.startsAt);
        if (!isNaN(startDate.getTime())) {
          setStartTime(startDate.toTimeString().slice(0, 5)); // Extract HH:MM format
        }
      }
      if (q?.endsAt) {
        const endDate = new Date(q.endsAt);
        if (!isNaN(endDate.getTime())) {
          setEndTime(endDate.toTimeString().slice(0, 5)); // Extract HH:MM format
        }
      }
      ...
      displayMessage(e?.message || "Failed to load quiz", "error");
    }
  };
          ...
          <div className="bg-white border border-gray-200 rounded-md p-4 w-full">
            <div className="flex items-center justify-between mb-3">
              ...
              <Button
                variant="contained"
                size="small"
                onClick={() => { setEditingIndex(null); setShowQuestionModal(true); }}
                        ...
                        <Stack direction="row" spacing={1} sx={{ mt: 0.5 }}>
                          <Tooltip title="Edit question">
                            <IconButton edge="end" aria-label="edit" onClick={() => {
                              ...
                              setQuestionName(qx.questionName);
                              setOptionA(qx.options.A);
                              setOptionB(qx.options.B);
                              setOptionC(qx.options.C);
                              setOptionD(qx.options.D);
                              setCorrectQuestion(qx.correctQuestion);
                              setScore(String(qx.score));
                              setShowQuestionModal(true);
                            }} sx={{ bgcolor: 'warning.main', color: 'common.white', borderRadius: '50%', '&:hover': { bgcolor: 'warning.dark' } }}>
                          ...
                          <Tooltip title="Remove question">
                            ...
                            }} sx={{ bgcolor: 'error.main', color: 'common.white', borderRadius: '50%', '&:hover': { bgcolor: 'error.dark' } }}>
      ...
      {showQuestionModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-greyBlack">{editingIndex !== null ? 'Update Question' : `Add Question ${questions?.length + 1}`}</h3>
                <button
              ...
              <div onKeyDown={(e) => { if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') { e.preventDefault(); handleAddQuestion(); } }}>
                ...
                <div className="flex gap-3 justify-end">
                  ...
                  <Button variant="contained" color={editingIndex !== null ? 'warning' : 'primary'} onClick={handleAddQuestion} sx={{ textTransform: 'none' }}>{editingIndex !== null ? 'Update' : 'Add Question'}</Button>
                </div>
              </div>
            </div>
          </div>
