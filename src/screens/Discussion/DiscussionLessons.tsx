...
const DiscussionLessons: React.FC = () => {
    ...
    const [loadingThreadCounts, setLoadingThreadCounts] = useState(false);
    ...
    useEffect(() => {
        ...
        setLoading(true);
        Get(`/discussions/topics/${topicId}/lessons`)
            .then((d) => {
    ...
    useEffect(() => {
        ...
        Get(`/discussions/subjects/${subjectId}/topics`)
            .then((d) => {
                if (d.success) {
                    const topicData = d.data?.find((t: any) => t._id === topicId);
    ...
    // Load thread counts for each lesson
    useEffect(() => {
        ...
        const loadThreadCounts = async () => {
            setLoadingThreadCounts(true);
            const counts: Record<string, number> = {};

            // Load all thread counts in parallel for better performance
            const promises = lessons.map(async (lesson) => {
                try {
                    const response = await Get(`/discussions/lessons/${lesson._id}/threads`);
                    if (response.success && Array.isArray(response.data)) {
                        return { lessonId: lesson._id, count: response.data.length };
                    } else {
                        return { lessonId: lesson._id, count: 0 };
                    }
                } catch (error) {
                    return { lessonId: lesson._id, count: 0 };
                }
            ...
            try {
                ...
                results.forEach(({ lessonId, count }) => {
                ...
                lessons.forEach(lesson => {
                    fallbackCounts[lesson._id] = 0;
                });
                setThreadCounts(fallbackCounts);
            } finally {
                setLoadingThreadCounts(false);
            }
        };
    ...
    const formatThreadCount = (lessonId: string) => {
        const count = threadCounts[lessonId] || 0;
                ...
                <div className="max-w-6xl mx-auto w-full">
                    <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 rounded-2xl shadow-2xl p-8 mb-6 relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            <h1 className="text-2xl font-bold text-white">Select a Lesson</h1>
                            <p className="text-slate-300">Choose a lesson under {toTitleCase(topic?.name || 'Topic')} to view its discussions</p>
                        </div>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div className="mb-4">
                            <nav className="text-sm text-slate-500">
                                ...
                                <span className="mx-2">/</span>
                                <span className="text-slate-700 font-medium">{toTitleCase(topic?.name || 'Topic')}</span>
                            </nav>
                        </div>
                    )}
                        ...
                        {loading ? (
                            ...
                            <div className="text-center py-16">
                                <div className="max-w-md mx-auto">
                                    ...
                                    {/* Title */}
                                    <h3 className="text-xl font-semibold text-slate-800 mb-3">No Lessons Available</h3>

                                    {/* Description */}
                                    <p className="text-slate-600 mb-6 leading-relaxed">
                                        There are no lessons available under this topic yet.
                                        Lessons will appear here once they are created by your administrator.
                                    </p>
                                    ...
                                    {lessons.map((l: any) => (
                                        <li key={l._id} className="py-3">
                                            <button
                                                ...
                                                onClick={() => navigate(`/Discussion/${subjectId}/${topicId}/${l._id}`)}
                                            >
                                                <div className="flex items-start gap-3">
                                                    <div className="flex-1">
                                                        ...
                                                        <div className="text-slate-500 text-sm mt-0.5">
                                                            {loadingThreadCounts ? <SkeletonLoader /> : formatThreadCount(l._id)}
                                                        </div>
                                                    </div>
                                                </div>
                                            </button>
...
export default DiscussionLessons;
